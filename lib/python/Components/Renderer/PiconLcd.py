...
class PiconLcd(Renderer):
	def __init__(self):
		...
		if not pngname:
			tmp = resolveFilename(SCOPE_CURRENT_SKIN, "picon_lcd_default.png")
			if pathExists(tmp):
				pngname = tmp
			else:
				pngname = resolveFilename(SCOPE_SKIN_IMAGE, "skin_default/picon_lcd_default.png")
