...
class DVDOverlay(Screen):
	def __init__(self, session, args = None, height = None):
		...
		if height is not None:
			h = height
		DVDOverlay.skin = """<screen name="DVDOverlay" position="0,0" size="%s,%s" flags="wfNoBorder" zPosition="-1" backgroundColor="transparent" />""" %(w, h)
		Screen.__init__(self, session)
...
def Plugins(**kwargs):
	list = [
		PluginDescriptor(name = "DVDPlayer", description = "Play DVDs", where = PluginDescriptor.WHERE_PLUGINMENU, needsRestart = False, fnc = main),
